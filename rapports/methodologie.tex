...
\begin{center}
	\begin{algorithm}[H] \label{alg:metho}
		\SetAlgoLined
		\KwIn{Ensemble de données d'entraînement avec une proportion $p$ d'anomalies $\boldsymbol{x^{(i)}}, i=1,...,n$ \\ Ensemble de données de test avec une proportion $p^*$  d'anomalies $\boldsymbol{x^{*(j)}}, j=1, ..., k$
