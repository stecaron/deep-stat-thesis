...
Le cœur de notre travail consiste à proposer une méthode de détection d'anomalies où nous utilisons la représentation latente d'un autoencodeur variationnel. Cette représentation servira ensuite comme base pour notre cadre décisionnel nous permettant de détecter des anomalies dans un \DIFdelbegin \DIFdel{jeu }\DIFdelend \DIFaddbegin \DIFadd{ensemble }\DIFaddend de données \DIFaddbegin \DIFadd{constitué d'images}\DIFaddend .

\section{Les objectifs de l'approche} \label{objectifs_metho}

Le premier objectif de notre approche est de faire la détection d'anomalies sur des données complexes, \DIFdelbegin \DIFdel{comme }\DIFdelend par exemple des images. En second lieu, nous souhaitons avoir une approche qui discrimine des anomalies en utilisant un seuil qui s'apparente à un niveau de confiance plutôt qu'\DIFaddbegin \DIFadd{à }\DIFaddend une métrique quelconque qui peut être difficile à établir et à interpréter. Sachant ces \DIFdelbegin \DIFdel{2 }\DIFdelend objectifs, il faut que notre méthodologie soit en mesure de traiter des données complexes et doit nous donner en sortie un score d'anomalie facile à interpréter.
...
Ensuite, l'avantage d'avoir un score d'anomalie facile à interpréter en sortie est de simplifier la prise de décision pour discriminer les anomalies des observations \DIFdelbegin \DIFdel{normales}\DIFdelend \DIFaddbegin \DIFadd{"normales"}\DIFaddend . Pour illustrer cet avantage, prenons un contre-exemple où l'on veut faire la détection d'anomalies à partir d'une méthode basée sur une distance comme les $k$ plus proches voisins. Dans cette approche, nous pourrions considérer comme \DIFdelbegin \DIFdel{anormales }\DIFdelend \DIFaddbegin \DIFadd{"anormales" }\DIFaddend les observations pour lesquelles il y a moins de $k$ voisins à l'intérieur d'un rayon de longueur $\epsilon$. La détection d'anomalie requiert donc de définir les paramètres $k$ et $\epsilon$, qui ne pourront pas être choisis aussi facilement qu'un niveau de confiance. Un score d'anomalie que l'on pourrait interpréter de manière similaire à un niveau de confiance $\alpha$ serait effectivement beaucoup plus simple et objectif à déterminer.

\section{Les hypothèses de l'approche}

Dans notre approche, nous supposons que  nous avons accès à un jeu de données d'entraînement $\mathcal{X} = \{\boldsymbol{X^{(1)}}, ..., \boldsymbol{X^{(n)}}\}$ qui contient $n$ observations indépendantes de $\mathbf{X} \in \mathbb{R}^{d_1 \times d_2}$. Dans notre cas, on suppose que les matrices sont carrées, soit $d_1=d_2=d$. Pour simplifier la notation, ces matrices peuvent être exprimées comme des vecteurs de longueur $d^2$. On peut donc réécrire sous la forme vectorielle $\boldsymbol{x^{(i)}} = [x^{(i)}_1,...,x^{(i)}_{d^2}]$; la façon d'obtenir cette forme vectorielle a peu d'importance, tant qu'on procède toujours de la même manière pour toutes les observations. Ces représentations peuvent être obtenues par ligne ou par colonne. Dans notre cas, nous avons dû procéder à ce genre de transformation pour l'une des approches comparatives testées dans nos applications, soit l'analyse en composantes principales\DIFdelbegin \DIFdel{à noyau}\DIFdelend . Pour ce faire, nous avons \DIFdelbegin \DIFdel{aplatit }\DIFdelend \DIFaddbegin \DIFadd{aplati }\DIFaddend les matrices (images) en vecteurs. La figure \ref{fig:one_hot} illustre \DIFdelbegin \DIFdel{comment nous avons procédé pour les }\DIFdelend \DIFaddbegin \DIFadd{de quelle façon nous pourrions procéder pour des }\DIFaddend images à un seul canal et \DIFdelbegin \DIFdel{les images }\DIFdelend \DIFaddbegin \DIFadd{des images à trois canaux, ou }\DIFaddend de type \textit{RGB}.
...
Les $n$ observations de notre jeu d'entraînement proviennent d'un mélange à proportion $(1-p)$ d'observations identiquement distribuées dites "normales" et $p$ d'observations identiquement distribuées dites \DIFdelbegin \DIFdel{anormales}\DIFdelend \DIFaddbegin \DIFadd{"anormales"}\DIFaddend . Pour indiquer si la matrice $\boldsymbol{X^{(i)}}$ provient de la population dite "normale" $\mathcal{N}$, soit $ \boldsymbol{X^{(i)}}\in\mathcal{N}$, on utilise la fonction indicatrice définie ci-dessous,
...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{Figure montrant le mécanisme derrière le concept de \textit{perceptual optimization}. Au lieu de calculer la perte entre $\boldsymbol{x}$ et $\hat{\boldsymbol{x}}$, la perte est calculée entre $g(\boldsymbol{x})$ et $g(\hat{\boldsymbol{x}})$. La fonction \DIFdelbeginFL \DIFdelFL{$g(a)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$g(\cdot)$ }\DIFaddendFL permet d'extraire les valeurs d'une couche spécifique d'un autre réseau pré-entraîné, comme par exemple \textit{VGG16} pré-entraîné sur \textit{ImageNet}.}
	\label{fig:perceptual}
\end{figure}

Étant donné que le réseau correspondant à la fonction \DIFdelbegin \DIFdel{$g(a)$ }\DIFdelend \DIFaddbegin \DIFadd{$g(\cdot)$ }\DIFaddend dans la figure \ref{fig:perceptual} est pré-entraîné sur plusieurs images réelles, cela nous permet d'extraire une couche qui contient de l'information à plus bas niveau comme la reconnaissance de lignes, des formes précises, des couleurs, etc. Plus la couche sélectionnée est près de l'entrée du réseau, plus l'information utilisée pour calculer la perte sera simple et brute. À l'inverse, si on sélectionne une couche plus près de la sortie, nous serons en mesure de prendre en compte de l'information plus complexe, par \DIFdelbegin \DIFdel{exemples }\DIFdelend \DIFaddbegin \DIFadd{exemple }\DIFaddend des formes particulières \citep{Johnson2016Perceptual}. L'erreur de reconstruction qui était définie à l'équation \ref{eq:mse_loss}, peut maintenant se définir comme suit:
...
\begin{figure} [h]
	...
	\begin{subfigure}{6cm}
		...
		\caption{Scénario 2}
	\end{subfigure}
	\caption{Exemples de situations représentant les \DIFdelbeginFL \DIFdelFL{2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{deux }\DIFaddendFL différents scénarios se rapportant  aux représentation latentes des observations "normales" et "anormales".}
	\label{fig:scenarios}
\end{figure}
...
%DIFDELCMD < 		\big\{t \in \mathbb{R}: t < s_{(1 - \alpha)} \big\} & \text{si les observations "normales" sont près de $N(0, I)$} \\
%DIFDELCMD < 		\big\{t \in \mathbb{R}: t > s_{(1 - \alpha)} \big\} & \text{si les observations "normales" sont éloignées de $N(0, I)$} 
%DIFDELCMD < 	\end{cases}
%DIFDELCMD < %%%
...
Pour \DIFdelbegin \DIFdel{tester }\DIFdelend \DIFaddbegin \DIFadd{étiqueter }\DIFaddend les observations de notre jeu de données test, nous allons calculer les statistiques de distance $T$ des $k$ observations de ce jeu de données $\mathcal{X^*}$. Si la valeur de la statistique de distance $T^{(j)}$ correspondant à l'observation $\boldsymbol{x^{*(j)}}$ fait partie de la région $R_N$, on peut donc conclure que l'observation fait partie de \DIFdelbegin \DIFdel{l}\DIFdelend \DIFaddbegin \DIFadd{"}\DIFaddend 'ensemble $(1-\alpha)$-normal\DIFaddbegin \DIFadd{"}\DIFaddend . Si la statistique de distance $T^{(j)}$ ne fait pas partie de la région $R_N$, on l'étiquettera comme une \DIFdelbegin \DIFdel{anomalie}\DIFdelend \DIFaddbegin \DIFadd{"$\alpha$-anomalie"}\DIFaddend .

La région $R_N$ nous permet de définir si une observation est une $\alpha$-anomalie ou non. De manière similaire, on peut également utiliser l'ensemble $T_{\mathcal{X}}$ pour établir un score d'anomalie pour une observation donnée. Ce score d'anomalie est défini entre $[0,1]$, où une anomalie évidente devrait avoir un score se rapprochant de 1. Pour établir ce score, il \DIFdelbegin \DIFdel{suffit de }\DIFdelend \DIFaddbegin \DIFadd{faut }\DIFaddend calculer la statistique de distance $T^{(j)}$ correspondant à l'observation $\boldsymbol{x^{*(j)}}$, ensuite \DIFdelbegin \DIFdel{de }\DIFdelend calculer le rang de cette statistique de distance parmi l'ensemble $T^{'}_{\mathcal{X}}$ et finalement diviser ce rang par le nombre d'observations dans l'ensemble $T^{'}_{\mathcal{X}}$, soit $n$ dans notre cas. Ce score d'anomalie, défini comme $\gamma$, peut \DIFaddbegin \DIFadd{donc }\DIFaddend être obtenu pour une observation $\boldsymbol{x^{*(j)}}$ \DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{par
}\DIFaddend 
...
\begin{center}
	\begin{algorithm}[H] \label{alg:metho}
		\SetAlgoLined
		\DIFdelbegin %DIFDELCMD < \KwIn{Ensemble de données d'entraînement avec une proportion $p$ d'anomalies $\boldsymbol{x^{(i)}}, i=1,...,n$ \\ Ensemble de données de test avec anomalies $\boldsymbol{x^{*(j)}}, j=1, ..., k$
%DIFDELCMD < 		\\ Niveau de filtration $\alpha$}
%DIFDELCMD < 		\KwOut{Indicateurs d'anomalies $o^{(j)}, j=1,...,k$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \KwIn{Ensemble de données d'entraînement avec une proportion $p$ d'anomalies $\boldsymbol{x^{(i)}}, i=1,...,n$ \\ Ensemble de données de test avec une proportion $p^*$  d'anomalies $\boldsymbol{x^{*(j)}}, j=1, ..., k$
		\\ Niveau de filtration $\alpha$}
		\KwOut{Indicateurs d'anomalies $\hat{\delta}^{(j)}, j=1,...,k$}
		\DIFaddend $\theta$, $\phi$ $\leftarrow$ paramètres de l'encodeur ($q_{\theta}(\cdot)$) et du  décodeur ($p_{\phi}(\cdot)$) du VAE entraîné\;
		\For{i=1 to n}{
		...
		\DIFdelbegin %DIFDELCMD < \For{j=1 to k}{
%DIFDELCMD < 			$(\boldsymbol{\mu^{(j)}}, \boldsymbol{\sigma^{(j)}}) = q_{\theta}(\boldsymbol{x^{*(j)}})$ \\
%DIFDELCMD < 			$T_{\mathcal{X^*}}^{(j)}=D_{KL}\big[N(\boldsymbol{\mu^{(j)}}, \boldsymbol{\sigma^{(j)}}) || N(0, I)\big]$ \\
%DIFDELCMD < 			$\gamma(\boldsymbol{x^{*(j)}}) = rang_{T^{'}_{\mathcal{X}}}(T_{\mathcal{X^*}}^{(j)})/n$ \\
%DIFDELCMD < 			\eIf{$\gamma(\boldsymbol{x^{*(j)}}) >= (1 - \alpha)$}{
%DIFDELCMD < 				$o^{(j)}$ = $vrai$
%DIFDELCMD < 			}
%DIFDELCMD < 			{
%DIFDELCMD < 				$o^{(j)}$ = $faux$
%DIFDELCMD < 			}
%DIFDELCMD < 		}
%DIFDELCMD < 		\KwRet{$\boldsymbol{o}$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \For{j=1 to k}{
		...
		\KwRet{$\boldsymbol{\hat{\delta}}$}
		\DIFaddend \caption{Algorithme de détection d'anomalies basé sur les représentations latentes d'un autoencodeur variationnel}
	\end{algorithm}
\end{center}
