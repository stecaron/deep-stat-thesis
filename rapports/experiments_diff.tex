...
Le tableau \ref{tab:results_cars} synthétise les résultats obtenus sur le jeu de données provenant de \textit{ImageNet}. On peut y voir les résultats pour les 4 approches différentes, pour les 3 scénarios de contamination ainsi que pour les 3 métriques choisies. Nous avons mis en gras les méthodes pour lesquelles les résultats sont significativement supérieurs aux autres selon un test de Mann-Whitney, aussi appelé \textit{Wilcoxon Rank Sum Test} \citep{Mann47}. Ce test statistique non paramétrique permet de tester l'hypothèse \DIFdelbegin \DIFdel{qu}\DIFdelend \DIFaddbegin \DIFadd{que le score médian ou moyen d}\DIFaddend 'un groupe est \DIFdelbegin \DIFdel{significativement }\DIFdelend supérieur à \DIFdelbegin \DIFdel{un autre. Pour l'adapter }\DIFdelend \DIFaddbegin \DIFadd{celui d'un autre groupe. Étant donné que le test permet de comparer un groupe de valeurs }\DIFaddend à \DIFdelbegin \DIFdel{notre besoin}\DIFdelend \DIFaddbegin \DIFadd{un autre groupe de valeurs}\DIFaddend , nous avons \DIFdelbegin \DIFdel{comparé la méthode avec la }\DIFdelend \DIFaddbegin \DIFadd{testé les 20 valeurs d'une }\DIFaddend métrique \DIFdelbegin \DIFdel{moyenne la plus élevée avec les 3 autres méthodes de façon séparée. Ce test ne nous permet donc pas de conclure si une }\DIFdelend \DIFaddbegin \DIFadd{obtenue par une }\DIFaddend méthode \DIFdelbegin \DIFdel{est significativement supérieure à toutes les autres, mais nous permet quand }\DIFdelend \DIFaddbegin \DIFadd{avec les 20 valeurs de cette }\DIFaddend même \DIFdelbegin \DIFdel{de valider qu'une }\DIFdelend \DIFaddbegin \DIFadd{métrique, mais obtenues par une autre }\DIFaddend méthode\DIFdelbegin \DIFdel{est supérieure aux 3 autres individuellement}\DIFdelend . Nous avons sélectionné un seuil de significativité de $5 \%$ pour le test. \DIFaddbegin \DIFadd{Dans le cas où une méthode est en gras, cela veut dire qu'elle est significativement supérieure à toutes les autres méthodes prises une à la fois. }\DIFaddend Si aucune méthode n'est en gras, cela veut dire qu'aucune \DIFaddbegin \DIFadd{ne }\DIFaddend se démarque de manière significative \DIFdelbegin \DIFdel{à }\DIFdelend \DIFaddbegin \DIFadd{de }\DIFaddend toutes les autres. 
