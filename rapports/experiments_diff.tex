...
où $l(x^{*(j)}, \hat{x}^{*(j)})$ correspond à l'erreur de reconstruction pour l'observation $i$ du jeu de données test $\mathcal{X^*}$ et $t_{1-\alpha}(L(\mathcal{X}, \hat{\mathcal{X}}))$ correspond au percentile $1-\alpha$ de l'ensemble des erreurs de reconstruction sur le jeu de données d'entraînement $\mathcal{X}$. \DIFdelbegin \DIFdel{Le niveau de filtration $\alpha$ choisi est le même que la proportion de contamination dans l'ensemble de données de test. }\DIFdelend Dans le cas de l'ACP appliquée sur \textit{ImageNet}, nous avons calculé l'erreur de reconstruction sur chaque canal indépendamment pour ensuite prendre la moyenne des 3 erreurs de reconstruction. \DIFaddbegin \DIFadd{Le niveau de filtration $\alpha$ choisi est le même que la proportion de contamination dans l'ensemble de données de test. Nous avons fait cela afin de mettre toutes les méthodes comparatives sur le même pied d'égalité. Nous avons donc établi cette même valeur de $\alpha$ pour les 2 autres méthodes comparatives. Nous avons choisi $\alpha$ égal à la vraie proportion contamination du jeu de données test afin de se mettre dans une situation plus simple.
}\DIFaddend 

Pour le jeu de données \textit{ImageNet}, nous avons conservé le nombre de composantes principales nous permettant de conserver exactement 80 \% de la variabilité pour chaque canal RGB. Ce nombre de composantes principales peut donc varier légèrement selon le canal et l'échantillonnage du jeu de données \DIFdelbegin \DIFdel{. Ces nombres de composantes conservées peuvent varier }\DIFdelend \DIFaddbegin \DIFadd{et prend des valeurs }\DIFaddend entre 90 et \DIFdelbegin \DIFdel{110. }\DIFdelend \DIFaddbegin \DIFadd{110 dans cette application. }\DIFaddend Pour le jeu de données \textit{MINST}, nous avons toujours conservé les 30 premières composantes principales. Cela nous permet de conserver approximativement  80 \% de la variabilité\DIFdelbegin \DIFdel{, selon l'échantillonnage}\DIFdelend . 
...
\subsection{Résultats sur \textit{ImageNet}} \DIFaddbegin \label{results:imagenet}
\DIFaddend 

Le tableau \ref{tab:results_cars} synthétise les résultats obtenus sur le jeu de données provenant de \textit{ImageNet}. On peut y voir les résultats pour les 4 approches différentes, pour les 3 scénarios de contamination ainsi que pour les 3 métriques choisies. Nous avons mis en gras les \DIFaddbegin \DIFadd{méthodes pour lesquelles les }\DIFaddend résultats \DIFdelbegin \DIFdel{de la méthode nous apparaissant la plus adaptée pour chaque métrique. Ce choix est basé en grande partie sur la valeur moyenne de la métrique}\DIFdelend \DIFaddbegin \DIFadd{sont significativement supérieurs aux autres selon un test de Mann-Whitney, aussi appelé }\textit{\DIFadd{Wilcoxon Rank Sum Test}} \DIFadd{\mbox{%DIFAUXCMD
\citep{Mann47}}%DIFAUXCMD
. Nous avons sélectionné un niveau de confiance de $95 \%$ pour le test. Si aucune méthode est en gras, cela veut dire qu'aucune se démarque de manière significative aux autres}\DIFaddend .

\begin{table}[h]
	\centering
	\caption{Résultats selon les métriques d'aire sous la courbe ROC, de précision et de rappel pour les différentes expérimentations concernant le jeu de données \textit{ImageNet} \DIFaddbeginFL \DIFaddFL{(moyennes sur les 20 réplications $\pm$ un écart-type)}\DIFaddendFL .}
	\begin{tabular}{c|c|c c c c}
		...
		& $\text{Précision}_{\alpha=0.01}$ & 0.016 $\pm$ 0.028 & 0.019 $\pm$ 0.041 & 0.325 $\pm$ 0.268 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.400 \pm 0.231}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.400 \pm 0.231$  }\DIFaddendFL \\
		& $\text{Rappel}_{\alpha=0.01}$ & 0.025 $\pm$ 0.043 & 0.020 $\pm$ 0.040 & 0.089 $\pm$ 0.079 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.120 \pm 0.093}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.120 \pm 0.093$  }\DIFaddendFL \\
		\midrule
		\multirow{3}{*}{Égal} 
		& ROC & 0.426 $\pm$ 0.040 & 0.584 $\pm$ 0.050 & 0.859 $\pm$ 0.037 & $\mathbf{0.923 \pm 0.018}$  \\ 
		& $\text{Précision}_{\alpha=0.05}$ & 0.040 $\pm$ 0.023 & 0.136 $\pm$ 0.041 & 0.499 $\pm$ 0.086 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.504 \pm 0.057}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.504 \pm 0.057$  }\DIFaddendFL \\
		...
		& $\text{Précision}_{\alpha=0.1}$ & 0.111 $\pm$ 0.025 & 0.202 $\pm$ 0.035 & 0.437 $\pm$ 0.044 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.449 \pm 0.030}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.449 \pm 0.030$  }\DIFaddendFL \\
...
Dans la figure \ref{fig:auc_cars}, on peut voir plus en détails la distribution des résultats obtenus sur les 20 \DIFdelbegin \DIFdel{expérimentations }\DIFdelend \DIFaddbegin \DIFadd{réplications }\DIFaddend via des graphiques en boîtes et moustaches.
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Graphiques en boîtes et moustaches illustrant les résultats sur les 20 \DIFdelbeginFL \DIFdelFL{expérimentations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{réplications }\DIFaddendFL de chacune des approches et scénarios de contamination. Les 3 sous-figures concernent une métrique différente.}
	\label{fig:auc_cars}
\end{figure}

\subsection{MNIST}

Le tableau \ref{tab:auc_mnist} synthétise les résultats obtenus sur le jeu de données \textit{MNIST} pour la métrique d'aire sous la courbe ROC. Les résultats sont présentés par scénario de contamination et par scénario de test (voir tableau \ref{tab:mnist_scenarios} pour plus de détails sur les scénarios de test). Les résultats pour les métriques de précision et de rappel sont présentés de la même manière dans les tableaux \ref{tab:precision_mnist} et \ref{tab:recall_mnist} respectivement. Nous avons mis en gras les \DIFdelbegin \DIFdel{résultats de la méthode nous apparaissant la plus adaptée pour chacun des scénarios. Encore une fois, ce choix est basé en grande partie sur la valeur moyenne de la métrique}\DIFdelend \DIFaddbegin \DIFadd{méthodes qui sont significativement meilleures que toutes les autres. Nous avons utilisé le même test qu'expliqué à la sous-section \ref{results:imagenet}}\DIFaddend .

\begin{table}[h]
	\centering
	\caption{Résultats des aires sous la courbe ROC des différentes approches selon le scénario de contamination et le scénario de test appliqué sur le jeu de données \textit{MNIST} \DIFaddbeginFL \DIFaddFL{(moyennes sur les 20 réplications $\pm$ un écart-type)}\DIFaddendFL .}
	\begin{tabular}{c|c|c c c c}
		\toprule
		...
		& 2 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.988 \pm 0.005}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.988 \pm 0.005$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.988 \pm 0.005}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.988 \pm 0.005$ }\DIFaddendFL & $0.965 \pm 0.022$ & 0.982 $\pm$ 0.021   \\
		& 3 & 0.993 $\pm$ 0.005 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.994 \pm 0.004}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.994 \pm 0.004$ }\DIFaddendFL & $0.976 \pm 0.017$ & 0.992 $\pm$ 0.005   \\
		...
		\midrule
		\multirow{6}{*}{Égal} 
		& 1 & 0.993 $\pm$ 0.004 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.994 \pm 0.002}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.994 \pm 0.002$ }\DIFaddendFL & $0.980 \pm 0.011$ & $\mathbf{0.994 \pm 0.002}$   \\
		& 2 & 0.991 $\pm$ 0.003 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.992 \pm 0.002}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.992 \pm 0.002$ }\DIFaddendFL & $0.978 \pm 0.010$ & 0.988 $\pm$ 0.007   \\
		...
		& 6 & 0.929 $\pm$ 0.015 & 0.911 $\pm$ 0.027 & $0.882 \pm 0.031$ & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.933 \pm 0.028}$   }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.933 \pm 0.028$   }\DIFaddendFL \\
		\midrule
	\end{tabular} 
	\label{tab:auc_mnist}
\end{table}

Dans les figures \ref{fig:auc_mnist}, \ref{fig:precision_mnist} et \ref{fig:recall_mnist}, on peut voir plus en détails la distribution des résultats obtenus pour les 3 différentes métriques sur les 20 \DIFdelbegin \DIFdel{expérimentations }\DIFdelend \DIFaddbegin \DIFadd{réplications }\DIFaddend grâce à des graphiques en boîtes et moustaches.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{images/images_boxplots/auc_mnist.pdf}
	\caption{Graphiques en boîtes et moustaches illustrant les résultats sur les 20 \DIFdelbeginFL \DIFdelFL{expérimentations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{réplications }\DIFaddendFL de chacune des approches, des scénarios de test et des scénarios de contamination. Toutes les figures présentent les résultats en aire sous la courbe ROC.}
	\label{fig:auc_mnist}
\end{figure}

\begin{table}[h]
	\centering
	\caption{Résultats des précisions selon la quantité de contamination et le scénario de test sur le jeu de données \textit{MNIST}. La valeur de $\alpha$ dépend du scénario de contamination. Pour le scénario "Moins" $\alpha=0.01$, pour le scénario "Égal" $\alpha=0.05$ et finalement pour le scénario "Plus" $\alpha=0.1$ \DIFaddbeginFL \DIFaddFL{(moyennes sur les 20 réplications $\pm$ un écart-type)}\DIFaddendFL .}
	\begin{tabular}{c|c|c c c c }
		\toprule
		Contamination & Scénario & ACP & AE & ISOF-VAE & DA-VAE  \\
		\hline
		...
		& 2 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.500 \pm 0.424}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.500 \pm 0.424$ }\DIFaddendFL & 0.141 $\pm$ 0.178 & 0.238 $\pm$ 0.234 & 0.282 $\pm$ 0.243  \\
		& 3 & $\mathbf{0.583 \pm 0.482}$ & 0.411 $\pm$ 0.319 & 0.263 $\pm$ 0.289 & 0.387 $\pm$ 0.273  \\
		& 4 & 0.135 $\pm$ 0.182 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.197 \pm 0.193}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.197 \pm 0.193$ }\DIFaddendFL & 0.033 $\pm$ 0.057 & 0.020 $\pm$ 0.051  \\			
		& 5 & 0.110 $\pm$ 0.188 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.168 \pm 0.212}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.168 \pm 0.212$ }\DIFaddendFL & 0.028 $\pm$ 0.053 & 0.145 $\pm$ 0.159  \\
		& 6 & 0.118 $\pm$ 0.145 & $\mathbf{0.363 \pm 0.267}$ & 0.060 $\pm$ 0.092 & 0.115 $\pm$ 0.142  \\
		\midrule
		\multirow{6}{*}{Égal} 
		& 1 & 0.779 $\pm$ 0.079 & 0.777 $\pm$ 0.037 & 0.678 $\pm$ 0.102 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.809 \pm 0.057}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.809 \pm 0.057$  }\DIFaddendFL \\
		& 2 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.685 \pm 0.013}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.685 \pm 0.013$ }\DIFaddendFL & 0.672 $\pm$ 0.014 & 0.677 $\pm$ 0.011 & 0.681 $\pm$ 0.009  \\
		& 3 & 0.821 $\pm$ 0.056 & 0.793 $\pm$ 0.033 & 0.702 $\pm$ 0.089 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.829 \pm 0.059}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.829 \pm 0.059$  }\DIFaddendFL \\
		...
		\midrule
		\multirow{6}{*}{Plus} 
		& 1 & 0.535 $\pm$ 0.016 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.563 \pm 0.020}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.563 \pm 0.020$ }\DIFaddendFL & 0.545 $\pm$ 0.021 & 0.555 $\pm$ 0.027  \\
		& 2 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.583 \pm 0.024}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.583 \pm 0.024$ }\DIFaddendFL & 0.566 $\pm$ 0.021 & 0.550 $\pm$ 0.036 & 0.556 $\pm$ 0.025  \\
		...
		& 6 & 0.425 $\pm$ 0.023 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.464 \pm 0.022}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.464 \pm 0.022$ }\DIFaddendFL & 0.394 $\pm$ 0.044 & 0.456 $\pm$ 0.036  \\
		\midrule
	\end{tabular} 
...
\begin{figure}[H]
	...
	\caption{Graphiques en boîtes et moustaches illustrant les résultats sur les 20 \DIFdelbeginFL \DIFdelFL{expérimentations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{réplications }\DIFaddendFL de chacune des approches, des scénarios de test et des scénarios de contamination. Toutes les figures présentent les résultats en précision.}
...
\begin{table}[h]
	...
	\begin{tabular}{c|c|c c c c }
		...
		\hline
		\multirow{6}{*}{Moins} 
		& 1 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.200 \pm 0.139}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.200 \pm 0.139$ }\DIFaddendFL & 0.138 $\pm$ 0.067 & 0.095 $\pm$ 0.092 & 0.075 $\pm$ 0.062  \\
		& 2 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.100 \pm 0.085}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.100 \pm 0.085$ }\DIFaddendFL & 0.081 $\pm$ 0.107 & 0.085 $\pm$ 0.079 & 0.1 $\pm$ 0.095  \\
		& 3 & 0.131 $\pm$ 0.134 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.181 \pm 0.170}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.181 \pm 0.170$ }\DIFaddendFL & 0.09 $\pm$ 0.118 & 0.11 $\pm$ 0.094  \\
		& 4 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.100 \pm 0.116}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.100 \pm 0.116$ }\DIFaddendFL & 0.088 $\pm$ 0.089 & 0.038 $\pm$ 0.070 & 0.025 $\pm$ 0.063  \\			
		& 5 & 0.063 $\pm$ 0.093 & 0.075 $\pm$ 0.108 & 0.031 $\pm$ 0.054 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.088 \pm 0.089}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.088 \pm 0.089$  }\DIFaddendFL \\
		& 6 & 0.081 $\pm$ 0.099 & $\mathbf{0.156 \pm 0.130}$ & 0.056 $\pm$ 0.084 & 0.0625 $\pm$ 0.074  \\
		\midrule
		\multirow{6}{*}{Égal} 
		& 1 & 0.859 $\pm$ 0.083 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.871 \pm 0.067}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.871 \pm 0.067$ }\DIFaddendFL & 0.663 $\pm$ 0.110 & 0.778 $\pm$ 0.066  \\
		& 2 & \DIFdelbeginFL \DIFdelFL{$\mathbf{1.000 \pm 0.000}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.000 \pm 0.000$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$\mathbf{1.000 \pm 0.000}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.000 \pm 0.000$ }\DIFaddendFL & 1.000 $\pm$ 0.001 & 1.000 $\pm$ 0.001  \\
		& 3 & 0.873 $\pm$ 0.079 & \DIFdelbeginFL \DIFdelFL{$\mathbf{0.901 \pm 0.038}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.901 \pm 0.038$ }\DIFaddendFL & 0.671 $\pm$ 0.116 & 0.786 $\pm$ 0.064  \\
...
\begin{figure}[H]
	...
	\caption{Graphiques en boîtes et moustaches illustrant les résultats sur les 20 \DIFdelbeginFL \DIFdelFL{expérimentations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{réplications }\DIFaddendFL de chacune des approches, des scénarios de test et des scénarios de contamination. Toutes les figures présentent les résultats en rappel.}
...
Dans la sous-section précédente, nous avons mentionné que les approches basées sur la reconstruction, soit ACP et AE, sont beaucoup moins performantes que les deux autres approches. D'ailleurs, on remarque que la méthode ACP obtient des aires sous la courbe ROC sous 0.5 pour tous les scénarios de contamination. En théorie, nous aurions donc pu, pour cette méthode, obtenir de meilleurs résultats en calculant notre score d'anomalie comme l'inverse du score actuel. Actuellement, notre prémisse de base est que les images étant les moins bien reconstruites sont potentiellement des anomalies. À l'inverse, les images les mieux reconstruites sont potentiellement des images "normales". Nous avons fait cette hypothèse puisque les anomalies sont peu fréquentes dans nos jeux de données, ce qui devrait faire en sorte que les paramètres appris par le modèle \DIFdelbegin \DIFdel{soit }\DIFdelend \DIFaddbegin \DIFadd{soient }\DIFaddend davantage représentés par les images "normales". En analysant les résultats en aire sous la courbe ROC obtenus par l'ACP, nous concluons que c'est plutôt les anomalies qui \DIFaddbegin \DIFadd{sont }\DIFaddend mieux reconstruites que les images "normales". Pour s'en convaincre, nous avons regardé des exemples d'images les mieux reconstruites sur l'ensemble d'entraînement. On peut voir quelques exemples à la figure \ref{fig:acp_reconstructionsa}. Dans le même ordre d'idées, la figure \ref{fig:acp_reconstructionsb} illustre quelques exemples d'images qui \DIFdelbegin \DIFdel{les }\DIFdelend \DIFaddbegin \DIFadd{sont }\DIFaddend moins bien reconstruites. 
...
En analysant la figure \ref{fig:acp_reconstructions}, on remarque que les images bien reconstruites sont des images relativement "simples". Les images de la classe "normale" à la figure \ref{fig:acp_reconstructionsa} montrent des voitures avec une vue relativement éloignée. Les arrières plans sont également plutôt \DIFdelbegin \DIFdel{simple}\DIFdelend \DIFaddbegin \DIFadd{simples}\DIFaddend , avec des couleurs assez unies. À l'inverse, les images les moins biens reconstruites (figure \ref{fig:acp_reconstructionsb}) sont plus "complexes". Par exemple, on voit les voitures de très près, avec beaucoup de détails. Aussi, les arrières plans sont beaucoup plus complexes, avec différentes couleurs et formes. Au final, on pourrait conclure que la méthode basée sur l'ACP discrimine sur la complexité de l'image plutôt que sur le contenu (voitures versus autres). La méthode AE performe un peu mieux que l'ACP, mais elle obtient tout de même des aires sous la courbe ROC inférieures à 0.6, et ce, pour les 3 scénarios de contamination. On pourrait penser que l'autoencodeur, avec la perte orientée sur le contenu \textit{perceptual loss}, gère un peu mieux ces différents éléments (vues, couleurs, arrière-plan, etc), mais cela n'est pas suffisant pour obtenir des performances optimales. Au final, les méthodes basées sur l'erreur de reconstruction nous apparaissent moins pertinentes lorsque nous devons faire la détection d'anomalies dans des images de complexités et de styles différents.
...
\begin{figure}[htb]
	...
	\caption{Graphique des 2 premières composantes principales \DIFdelbeginFL \DIFdelFL{réalisé sur les }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{calculées à partir des }\DIFaddendFL vecteurs $\boldsymbol{\mu}$ et $\boldsymbol{\sigma}$ du jeu de données d'entraînement de \textit{ImageNet} appliqué sur le scénario de contamination "Plus".}
	\label{fig:pca_cars}
\end{figure}
...
Dans la sous-section \ref{imagenet:reconsruction}, nous avions \DIFdelbegin \DIFdel{fais }\DIFdelend \DIFaddbegin \DIFadd{fait }\DIFaddend la conclusion que les méthodes basées sur la reconstruction ne permettaient pas d'obtenir de bonnes performances sur des images réelles. Dans le cas du jeu de données \textit{MNIST}, les performances de ces méthodes sont généralement supérieures à \DIFdelbegin \DIFdel{ces }\DIFdelend \DIFaddbegin \DIFadd{celles }\DIFaddend basées sur la représentation latente. Pour se convaincre de ces résultats, la figure \ref{fig:mnist_acp_reconstructionsa} montre des exemples d'images du jeu de données d'entraînement ayant de petites erreurs de reconstruction. Ces exemples proviennent de la méthode ACP et du scénario de test 6.  Dans le même ordre d'idées, la figure \ref{fig:mnist_acp_reconstructionsb} \DIFdelbegin \DIFdel{illustre }\DIFdelend \DIFaddbegin \DIFadd{montre }\DIFaddend des images provenant de la même approche, mais ayant plutôt des erreurs de reconstruction élevées. 
...
Dans la section \ref{analyse_lat_cars}, nous avons fais l'analyse des représentations latentes par l'entremise des statistiques de distance et d'échantillons d'images. Cela nous a \DIFdelbegin \DIFdel{permit }\DIFdelend \DIFaddbegin \DIFadd{permis }\DIFaddend de conclure que les images "normales" avaient des statistiques de distance élevées alors que les anomalies avaient des statistiques de distance faibles. Il faut donc refaire cette analyse, mais dans le cas de \textit{MNIST}. Dans la figure \ref{fig:latentes_images_mnist}, on peut voir \DIFdelbegin \DIFdel{quelques uns }\DIFdelend \DIFaddbegin \DIFadd{quelques-uns }\DIFaddend des échantillons d'images du scénario de test 3 nous \DIFdelbegin \DIFdel{aidant }\DIFdelend \DIFaddbegin \DIFadd{aident }\DIFaddend à tirer notre conclusion. Cette fois-ci, on remarque que les observations ayant des statistiques de distance faibles proviennent de notre classe "normale", soit le chiffre "1". À l'inverse, les observations ayant des statistiques de distance élevées proviennent de notre classe "anormale", soit tous les autres chiffres. C'est d'ailleurs le cas pour les 6 différents scénarios de test.
...
Dans la figure \ref{fig:mnist_latent_2d}, nous avons utilisé \DIFaddbegin \DIFadd{comme exemple }\DIFaddend un entraînement \DIFaddbegin \DIFadd{réalisé }\DIFaddend sur scénario de test 3 avec le scénario de contamination "Égal". Afin de pouvoir mieux observer la distribution des points, nous avons conservé seulement 500 observations, choisies aléatoirement, du jeu de données d'entraînement. Comme nous l'avions remarqué dans la figure \ref{fig:mnist_latent_stats}, \DIFdelbegin \DIFdel{le }\DIFdelend \DIFaddbegin \DIFadd{les deux classes semblent mieux séparées dans }\DIFaddend vecteur latent $\boldsymbol{\sigma}$ \DIFdelbegin \DIFdel{semblent être plus discriminant que }\DIFdelend \DIFaddbegin \DIFadd{plutôt que dans }\DIFaddend le vecteur latent $\boldsymbol{\mu}$.
