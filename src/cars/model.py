...
class CarsConvAE(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.Conv2d(16, 128, kernel_size=3, stride=2, padding=1, bias=False), # b, 128, 64, 64
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1, bias=False), # b, 256, 32, 32
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, bias=False), # b, 512, 16, 16
            nn.BatchNorm2d(512),
            nn.ReLU())

        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(512, 256, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False), # b, 256, 32, 32
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.ConvTranspose2d(256, 128, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False), # b, 128, 64, 64
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.ConvTranspose2d(128, 16, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False), # b, 16, 128, 128
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.ConvTranspose2d(16, 3, kernel_size=3, stride=1, padding=1, bias=False), # b, 3, 128, 128
            nn.Sigmoid())
...
class VGG16ConvVAE(nn.Module):
    def __init__(self, z_dim, image_channels=256, h_dim=32896):
        super(VGG16ConvVAE, self).__init__()
        self.encoder = nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, bias=False), # b, 512, 16, 16
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.Conv2d(512, 1028, kernel_size=3, stride=2, padding=1, bias=False), # b, 1028, 8, 8
            nn.BatchNorm2d(1028),
            nn.ReLU(),
            nn.Conv2d(1028, 2056, kernel_size=3, stride=2, padding=1, bias=False), # b, 2056, 4, 4
            nn.BatchNorm2d(2056),
            nn.ReLU(),
            Flatten())
        ...
        self.decoder = nn.Sequential(
            UnFlatten(nb_filters=2056, size=4), # b, 2056, 4, 4
            nn.ConvTranspose2d(2056, 1028, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False), # b, 1028, 8, 8
            nn.BatchNorm2d(1028),
            nn.ReLU(),
            nn.ConvTranspose2d(1028, 512, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False), # b, 512, 16, 16
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.ConvTranspose2d(512, 256, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False), # b, 256, 32, 32
            nn.Sigmoid())
